...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # state = (pacmanPosition, cornersVisitedBoolean)
        # cornersVisitedBoolean = (c1, c2, c3, c4)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                # nextState = (nextXY positions, corner0, corner1, corner2, corner3)
                next_position = (nextx, nexty)
                corner0 = state[1][0]
                corner1 = state[1][1]
                corner2 = state[1][2]
                corner3 = state[1][3]
                next_state = (next_position, (corner0, corner1, corner2, corner3))
                cost = 1
                if next_position == self.corners[0]:
                    next_state = (next_position, (1, corner1, corner2, corner3))
                if next_position == self.corners[1]:
                    next_state = (next_position, (corner0, 1, corner2, corner3))
                if next_position == self.corners[2]:
                    next_state = (next_position, (corner0, corner1, 1, corner3))
                if next_position == self.corners[3]:
                    next_state = (next_position, (corner0, corner1, corner2, 1))
...
def cornersHeuristic(state, problem):
    ...
    # state = (pacmanPosition, corners_visited)
    #                      corners_visited = (c1, c2, c3, c4) boolean values


    # maximum of distances to corners
    # state[0] = current_pacman_position
    # mazeDistance0 = mazeDistance from pacmanPosition to corner0

    c0 = state[1][0]
    c1 = state[1][1]
    c2 = state[1][2]
    c3 = state[1][3]
    max_of_maze_distances = 0

    if c0 == 0:
        maze_distance0 = mazeDistance(state[0], corners[0], problem.startingGameState)
        max_of_maze_distances = max(max_of_maze_distances, maze_distance0)

    # mazeDistance1 = mazeDistance from corner0 to corner1
    if c1 == 0:
        maze_distance1 = mazeDistance(state[0], corners[1], problem.startingGameState)
        max_of_maze_distances = max(max_of_maze_distances, maze_distance1)

    # mazeDistance2 = mazeDistance from corner1 to corner2
    if c2 == 0:
        maze_distance2 = mazeDistance(state[0], corners[2], problem.startingGameState)
        max_of_maze_distances = max(max_of_maze_distances, maze_distance2)

    # mazeDistance3 = mazeDistance from corner2 to corner3
    if c3 == 0:
        maze_distance3 = mazeDistance(state[0], corners[3], problem.startingGameState)
        max_of_maze_distances = max(max_of_maze_distances, maze_distance3)

    return max_of_maze_distances
...
def foodHeuristic(state, problem):
    ...
    # Suggestion from Dan Klein's response in stack overflow
