...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for ghost_position in livingGhostPositionDistributions:
      most_likely_positions.append(ghost_position.argMax())

    # print "most_likely_positions", most_likely_positions

    for most_likely_position in most_likely_positions:
      if self.distancer.getDistance(pacmanPosition, most_likely_position) < min_dist:
        closest_ghost = most_likely_position
        for action in legal:
          successor = Actions.getSuccessor(pacmanPosition, action)
          if self.distancer.getDistance(successor, closest_ghost) < min_dist:
            best_action = action
            min_dist = self.distancer.getDistance(successor, closest_ghost)
