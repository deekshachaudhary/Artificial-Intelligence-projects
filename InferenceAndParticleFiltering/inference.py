...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    self.particles = []
    # print "numParticles", self.numParticles, "legalPositionsLength", len(self.legalPositions)
    for i in range(self.numParticles):
      self.particles.append(random.choice(self.legalPositions))
      # print "random.choice", random.choice(self.legalPositions)
  ...
  def observe(self, observation, gameState):
    ...
    pacmanPosition = gameState.getPacmanPosition()
    "*** YOUR CODE HERE ***"
    captured = allPossible = util.Counter()

    if noisyDistance == None:
      captured[self.getJailPosition()] = 1.0
      self.particles = captured

    else:
      for position in self.legalPositions:
        distance = util.manhattanDistance(position, pacmanPosition)
        allPossible[position] = emissionModel[distance] * self.getBeliefDistribution()[position]

      # if allPossible[position] == 0:
  ...
  def elapseTime(self, gameState):
    ...
    for oldPos in self.particles:
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      newPos.append(util.sample(newPosDist))
